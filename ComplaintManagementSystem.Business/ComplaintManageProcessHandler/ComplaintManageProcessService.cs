    ...
    public class ComplaintManageProcessService : IComplaintManageProcessService
    {
        ...
        public void CreateComplaint(IFormCollection collection, IFormFile file)
        {
            try
            {
                ...
                {
                    DepartmentComplainResolve(ProcessId, ResolvedRemark);
                }
        ...
        public async Task<PaginationResultsModel<ComplaintMaster>> getDepComplaintList(int pageNumber, int pageSize, string searchString)
        {
            try
            {
                string whereClause = "WHERE cmp.Active = 1 AND cmp.Status =1 AND cmp.IsSentDep =1 AND cmp.IsSentCentral = 0 AND (cmp.IsResolved IS NULL OR cmp.IsResolved <> 1)";
                if (!string.IsNullOrEmpty(searchString))
                {
                    whereClause += " AND (cmp.Cus_Name LIKE @SearchPattern OR cmp.Refference LIKE @SearchPattern)";
                }
        ...
        public void UpdateComplaint(IFormCollection collection, IFormFile file)
        {
            try
            {
                ...
                var depQuery = $"UPDATE Complaint_ManageProcess SET Dep_Id={Convert.ToInt64(Dep_Id)} WHERE ComplaintMngProcess_Id={Convert.ToInt64(ComProcessId)} AND Active=1 AND EscalatiomMatrix=1";
        ...
        public void UpdateSendComplaint(IFormCollection collection, IFormFile file)
        {
            try
            {
                ...
                if (ResolvedStatus == "Yes")
                {
                    DepartmentComplainResolve(Convert.ToInt16(ComProcessId), ResolvedRemark);
                }
                return;
            }
            catch (Exception ex)
